...
#### 「触れる」「近くで見る」
「触れる」と「近くで見る」はインタラクションの一部です。アクション単体で見るとユニークさはなく、インパクトにも欠けています。
しかし、この2つのアクションは他の要素を尊重しており、ゲームに馴染むように設計されています。

#### 協調性のあるアクション
<img width = "600" src="Images/interaction_1.png">

**"コントローラーのボタン入力を使用しないことにより、没入感を損なわさせない"**
* ゲーム内で手に機械や装置を握っている場合はコントローラーのボタン入力を押すことに違和感は出ません。
* しかし、手に何も握っていない場合にボタン入力を行うことは違和感があり、途端に「ゲームチック」に感じます。これは没入感を低下させてしまいます。
* もちろん、没入感が多少低下するだけでは「フラットゲームと同じ」という答えにはなりませんが、
私は「VRゲームは細かい所を意識し丁寧に作ることによって、VRならではの体験にすることが出来る」と考えています。
