...
#### 4.3 イベント用システム
* 大まかなゲームシステムが決まった時に、イベントの呼び出しは使いまわすことが出来た方が制作速度が向上すると考えたため作成しました。
...
#### 3.1 お尻のぶつけ合い
* 私たちは、現実では出来ない体験がゲーム体験としてユーザーの興味を惹きやすいということに着目し、「現実では出来ない体験」を「今は出来ない体験」として発展させ、「大人になって遊ばなくなったが、子供の頃に遊んでいた遊び」を探しました。
* 遊びを探しているときに出会ったのが尻相撲です。Push Battleは尻相撲の「お尻のぶつけ合い」をゲーム体験として制作を始めました。

#### 3.2 突進お尻アタック(スマッシュ)

<img width = "500" src="Images/PushBattleSmash.gif">

* ただお尻をぶつけ合っているだけでは物足りないと感じたため、楽しく気持ちよいアクションとしてスマッシュが作られました。
* お尻の方向に突進するというアイデアは比較的簡単に出てきたのですが、いざ作ってみると、なかなか"しっくり"くるものにはなりませんでした。
* スマッシュは実際に私が動いて感じた「少し浮く」と、エビの動きから着想を得た「自然な減速」という二つの自然さを取り入れることによって、"しっくり"くる動きにすることが出来ました。

### 4 技術

#### 4.1 スマッシュの動き
* スマッシュはRigidbodyを使用し、減速はRigidbody.dragを使用しました。
