...
この経験により、ゲームデザイナーだけでなくプログラマーとしても学び、その学びをプロジェクトに活かしたいという気持ちが強く芽生えました。
...
* Unityには「ポストプロセスエフェクト」という機能があります。これを使用すればカメラエフェクトを実装することはできたのですが、使用すると少なからず処理負荷は増すため使用はしませんでした。
* 結果的に私は次の解決策をとりました。
...
**"シェーダーグラフ : 組み合わせ"**
* 私が新しい機能に出会ったときにすることは「とりあえず触る」です。触ったときの感想は「よく分からない。ＵＶって何だ？」でした。
...
**"シェーダーグラフ : 理解し作る"**
* 薄々気づいてはいたのですが、組み合わせによるシェーダーグラフの作成に限界が来ました。
* 理由は明確で、組み合わせに使用するシェーダーグラフが見つからなかったためです。

// add image damage

#### 視点の高さ補正
* 着席型のゲームを作る上で、プレイヤーの姿勢について考える必要がありました。
* UnityEngine.XRにはデバイスのトラッキング方法を選択できる機能があり、これはプレイヤーの姿勢に関与します。(TrackingOriginMode)
* 私はこの機能を使えば、工夫せずとも問題はないと考えていたのですが、悲しいことに問題は発生しました。

**"プレイヤーによって視点の高さが若干異なる問題"**
