...
「暗転」には以下の条件を満たせるため制作しました。
1. 汎用性が高い
2. 明示的である
3. 制作側にとって都合が良い
4. 違和感を与えない
* 1は汎用的であれば、あらゆる状況で使えるのでインタラクト対象別に時間をかける必要がなくなります。
これはインタラクションに必要とする時間を削減することで、中心的なゲームシステムのプロトタイプを作成する十分な時間を確保したかったためです。
* 2は明示的な反応を作ることによって、プレイヤーにインタラクトした手ごたえを与えるために必要でした。
インタラクションの反応が薄いと、プレイヤーは何を起こしたかを把握できなくなり、自身が目標の達成に近づいているのかを判断することができず、
