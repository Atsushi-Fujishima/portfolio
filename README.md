...
* コピペしたシステムを触ることによって、私が何を求めているかを明確させることが出来ました。そして、私の理解力では作成できない部分も分かりました。
 	* 求めているもの : 意図的な視点の揺れ, まっすぐ進める, 跳ねない, 片手だけでは移動させない
    * 作成できない部分 : Rigidbodyを使用したゲーム内の手のトラッキング(PID制御) 
* 求めているシステムを実現させるために、システムの新規実装とコピペシステムの一部を維持することを決定しました。

"ハイハイシステムの歴史 3 : 私のハイハイ"
* 最も困難であったのが「片手だけでは移動させない」でした。当時の私の知識は「一つの入力で一つを動かす」しかなかったためです。
* この問題を解決するために「ハイハイ」という動きをよく観察しました。気づいたことは、「左手と右手が交互に地面に着いたとき進める」ということでした。
* 気づきを実現するために、私の知識を全て展開し使えるものを探しました。
* 最初にたどり着いたのは「List」を使用することでした。これは後に「string」に変わります。
